...
export default function Dashboard() {
  ...
  async function reloadAll() {
    try {
      console.log("[dashboard] reloadAll");
      ...
      if (!usedV2) {
        console.log("[dashboard] V2 nicht genutzt → fallback auf v3");
  ...
  async function tryAnimeNetRecV2({
    ...
    try {
      console.log("[dashboard] versuche V2 zu laden …");
      ...
      console.log("[dashboard] V2 recommend() →", result.length, "Treffer");
      ...
      try {
        ...
        console.log("[dashboard] schreibe rec-event", ev);
        // direkt IPC
        await window.shokai.rec.insertEvent(ev);
      } catch (e) {
        console.warn("[dashboard] insertEvent hat nicht geklappt:", e);
      }
      ...
      console.warn("[dashboard] V2 nicht verfügbar:", e);
      return false;
    }
  }  
...
function sanitizeHtml(input?: string) {
  ...
  const out = Array.from(body.childNodes).map(walk).join("");
  return out || "Keine Beschreibung";
}
