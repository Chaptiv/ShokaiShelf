...
function sanitizeHtml(input?: string) {
  ...
  const allowed = new Set(["B", "STRONG", "I", "EM", "BR", "P", "UL", "OL", "LI", "SPAN", "A"]);
  const doc = new DOMParser().parseFromString(String(input), "text/html");
  function walk(n: Node): string {
    if (n.nodeType === Node.TEXT_NODE) return (n.textContent || "");
    if (n.nodeType === Node.ELEMENT_NODE) {
      const el = n as HTMLElement; const tag = el.tagName.toUpperCase();
      if (!allowed.has(tag)) return Array.from(el.childNodes).map(walk).join("");
      if (tag === "BR") return "<br/>";
      if (tag === "A") { const href = (el.getAttribute("href") || "#").replace(/"/g, "&quot;"); const inner = Array.from(el.childNodes).map(walk).join(""); return `<a href="${href}" target="_blank" rel="noopener noreferrer">${inner}</a>`; }
      const inner = Array.from(el.childNodes).map(walk).join(""); return `<${tag.toLowerCase()}>${inner}</${tag.toLowerCase()}>`;
    }
    return "";
  }
  const body = doc.body || doc; const out = Array.from(body.childNodes).map(walk).join(""); return out || "Keine Beschreibung";
}
function InfoDrawer({
